...
const WINDOWS_INSTALLER = {
  key: `windows/NexCompany-Setup-${VERSION}.exe`,
  contentType: "application/vnd.microsoft.portable-executable",
  filename: `NexCompany-Setup-${VERSION}.exe`,
};
...
const supabaseVerifyUrl = "https://cenayutywkiljwqyxfii.supabase.co/functions/v1/nexcompany-download";
...
export default {
  async fetch(request, env) {
    ...
    // completed NexCompany payment is confirmed.
    const sessionId = url.searchParams.get("session_id") ?? "";
    if (!/^cs_(test|live)_[A-Za-z0-9_]+$/.test(sessionId)) {
      return json({ error: "A completed NexCompany payment was not found" }, 402);
    }
    try {
      ...
      if (!verify.ok) {
        ...
        if (verify.status === 403 || verify.status === 404) return json({ error: "A completed NexCompany payment was not found" }, 402);
