...
const plans = new Map([
  [9900, { tier: "Personal", productName: "CJ PDF to Word — Personal" }],
  [19900, { tier: "Professional", productName: "CJ PDF to Word — Professional" }],
  [39900, { tier: "Business", productName: "CJ PDF to Word — Business" }],
]);
// Live Payment Link URLs are stored in Supabase secrets named after each tier.
const paymentLinkSecret = (tier: string) => `CJ PDF to Word — ${tier}`;
...
Deno.serve(async request => {
  ...
  // Checkout is OFF by default — set the secret CJ_PDF_CHECKOUT_ENABLED=true to switch it on.
  if (input.platform === "config") {
    if (Deno.env.get("CJ_PDF_CHECKOUT_ENABLED") !== "true") return respond(request, { mode: "disabled", plans: [] });
  ...
  const stripeKey = Deno.env.get("STRIPE_CJPDF_LIVE_SECRET_KEY") ?? Deno.env.get("STRIPE_SECRET_KEY") ?? "";
  ...
  try {
    ...
    if (!plan || !isValid) return respond(request, { error: "A completed CJ PDF to Word payment was not found" }, 403);
    return respond(request, { tier: plan.tier });
  } catch (error) {
    console.error("CJ PDF to Word download verification failed", error);
