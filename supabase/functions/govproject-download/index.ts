...
const plans = new Map([
  [49900, { tier: "Standard", productName: "GovProject PMIS — Standard" }],
  [89900, { tier: "Professional", productName: "GovProject PMIS — Professional" }],
  [149900, { tier: "Enterprise", productName: "GovProject PMIS — Enterprise" }],
]);
// Live Payment Link URLs are stored in Supabase secrets named after each tier.
const paymentLinkSecret = (tier: string) => `GovProject PMIS — ${tier}`;
...
Deno.serve(async request => {
  ...
  // Checkout is OFF by default — set the secret GOVPROJECT_CHECKOUT_ENABLED=true to switch it on.
  if (input.platform === "config") {
    if (Deno.env.get("GOVPROJECT_CHECKOUT_ENABLED") !== "true") return respond(request, { mode: "disabled", plans: [] });
  ...
  const stripeKey = Deno.env.get("STRIPE_GOVPROJECT_LIVE_SECRET_KEY") ?? Deno.env.get("STRIPE_SECRET_KEY") ?? "";
  ...
  try {
    ...
    if (!plan || !isValid) return respond(request, { error: "A completed GovProject PMIS payment was not found" }, 403);
    return respond(request, { tier: plan.tier });
  } catch (error) {
    console.error("GovProject PMIS download verification failed", error);
